    ...
    def scrape(self, parameters: Scraper.Parameters, progress: Progress) -> List[Article]:
        ...
        while page < 10: # gerade weil es sonst einfach zu lange dauert. TODO: andere Möglichkeit finden
            ...
            for item in data:
                ...
                title_tag = soup_payload.find("span", class_="bpa-teaser-title-text-inner")
                assert title_tag
                title = title_tag.get_text(strip=True) if title_tag else None
                if (not (title.startswith("Bundeskabinett - Ergebnisse")or title.startswith("Regierungspressekonferenz"))):
                    entries.append((timestamp,title,link))
            page +=1

        articles = []
        for timestamp, title, link in progress.start_iteration(entries, total=len(entries), desc="Scraping Breg articles"):
